	...
	$.fn.portfolioSetter = function (options) {
		...
		if (options.fullscreenMode) {
			...
			if (options.fullscreenable) {
				rootParent.after('<div class="EasyDNNGallery_fullscreen_btn_container"><a class="EasyDNNGallery_fullscreen_btn" href="#" onclick="eds1_8().fullscreenPortfolioGallery(\'' + rootParent.attr('id') + '\'); return false;">' + options.fullscreenButtonText + '</a></div>');
			}
		}
		...
		function generateItemObject(itemData) {
			...
			if (options.showSharingButtons.facebook || options.showSharingButtons.twitter || options.showSharingButtons.gplus) {
				...
				if (options.showSharingButtons.facebook) {
					socialSharing += '<iframe src="http://www.facebook.com/plugins/like.php?href=' + encodeURIComponent(share_url) + '&amp;layout=button_count&amp;show_faces=true&amp;action=like&amp;font&amp;colorscheme=light&amp;height=20" scrolling="no" frameborder="0" style="border:none; overflow:hidden; height:20px; width: 100px;" allowTransparency="true"></iframe>';
				}
				...
				if (options.showSharingButtons.twitter) {
					socialSharing += '<a href="http://twitter.com/share" class="twitter-share-button" data-url="' + share_url + '" data-count="horizontal">Tweet</a><script type="text/javascript" src="http://platform.twitter.com/widgets.js"></script>';
				}
