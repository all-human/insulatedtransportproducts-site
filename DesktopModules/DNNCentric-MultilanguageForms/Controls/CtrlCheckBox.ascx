<%@ Control Language="C#" AutoEventWireup="True" CodeBehind="CtrlCheckBox.ascx.cs"
    Inherits="DNNCentric.MultilanguageForms.Controls.CtrlCheckBox" %>
...
<tr><td colspan="2">&nbsp;</td></tr> 
    ...
    <tr>
        <td colspan="2">
            <asp:DataList ID="dlstChkBxOption" runat="server" Width="100%" BackColor="White"
                    ...
                    <table width="99%">
                        ...
                        <td>
                            <%--<asp:Label ID="lblDefault" runat="server" Text='<%#string.Format("{0} ({1})","Default Item",((mlContent)Container.DataItem).Culture) %>' CssClass="CF_Admin_NormalText"></asp:Label>--%>
                             <dnn:Label runat="server" ID="lblDefaultCheck" ResourceKey="lblDefaultCheck" CssClass="CF_Admin_NormalText" Text='<%#string.Format("{0} ({1})",Localization.GetString("lblDefaultCheck.DisplayText",UserResourceFile),((mlContent)Container.DataItem).Culture) %>' />
    ...
    <dnn:Label runat="server"  ID="lblMLHelpText" Visible="false" CssClass="CFC_NormalTextBox" Text="Tip to Enable MultiLanguage Feature." HelpText="<b>Tip:</b> Since you have enabled more than one language on your site,<br/>you can keep Field Properties in different languages.<br/>Go to [Form Layout] page to Enable MultiLanguage Feature."/>
    </td>
    </tr>
    <tr>
