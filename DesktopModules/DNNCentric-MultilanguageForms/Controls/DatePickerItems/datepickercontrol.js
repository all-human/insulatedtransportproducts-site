...
DatePickerControl.init = function()
{
	...
	for (i=0; i<inputsLength; i++){
		if (inputControls[i].type.toLowerCase() == "text"){
			...
			var dpcimagesrc = editctrl.getAttribute("datepicker_imagesrc");
			var setEvents = false;
			// if datepicker pseudo-attribute:
			if (dpcattr != null && dpcattr == "true"){
				if (editctrl.id){
					if (!this.createButton(editctrl, false,dpcimagesrc)) continue;
					setEvents = true;
				}
				else{
			...
			// if fomated id attr:
				...
				if (!this.createButton(editctrl, true,dpcimagesrc)) continue;
				setEvents = true;
			}
 ...
 */
DatePickerControl.createButton = function(input, useId,calimagesrc)
{
	...
	*/
	...
	calButton.style.width="25px";
	//calButton.src ="../../../DesktopModules/DNNCentric.MultilanguageForms/Controls/DatePickerItems/Calender.jpg";
	calButton.src =calimagesrc;
	if (this.buttonPosition == "in"){
...
DatePickerControl.getDateFromControl = function(ctrl)
{
	...
	var dpcattr   = ctrl.getAttribute("datepicker");
	//if (this.validate(ctrl.value, format))
...
DatePickerControl.reformatOnSubmit = function()
{
	...
	var i;
	for (i=0; i<inputsLength; i++){
		if (inputControls[i].type.toLowerCase() == "text" && inputControls[i].id.indexOf(this.editIdPrefix) == 0){
			var editctrl  = inputControls[i];
